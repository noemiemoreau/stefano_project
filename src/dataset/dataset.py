...
class ImageDataset(Dataset):
    ...
    def __init__(self, df, fn_col = None, lbl_col = None, transform = None, return_filename = False,
                 which_channels = [list(range(14))], data_path = None, data_suffixe = None):
        self.df = df
        ...
        self.data_suffixe = data_suffixe
    ...
    def __getitem__(self, idx):
        ...
        if self.data_path is not None and self.data_suffixe is not None:
            fn = self.data_path + os.path.basename(fn)[0:3] + self.data_suffixe
        image = np.load(fn)
